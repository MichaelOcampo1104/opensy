...
UniqueID : XMU_Chapter8_2
Author   : XMU (Xiamen University) — Chapter 8.2 Example
Date     : 2026-06-17
...
Ref      : XMU Finite Element Analysis course, Chapter 8.2
Units    : kN, m, sec, kPa  (coupled u-p — fluid properties are physical SI constants)
Notes    : Converted from model.tcl.
           Source uses standard PressureDependMultiYield (PDMY) with 16 parameters
           (minimum set — optional numYieldSurf, e, volLimits, and cohesion omitted).
           Parameter set differs substantially from Ch8.1: much stiffer soil
           (Gr=60000 kPa vs 506.6 kPa), lower friction angle (31° vs 47°).
"""
...
# --- Soil: PressureDependMultiYield (direct PDMY, 16-param minimum set) ---
pdmy_rho                = 2.0               # mass density  [Mg/m³]
pdmy_refShearModul      = 60000.0           # Gr  [kPa]
pdmy_refBulkModul       = 240000.0          # Br  [kPa]
pdmy_frictionAng        = 31.0              # φ  [°]
pdmy_peakShearStra      = 0.1               # peak shear strain
pdmy_refPress           = 80.0              # reference pressure  [kPa]
pdmy_pressDependCoe     = 0.5               # pressure dependency coefficient
pdmy_phaseTransformAng  = 26.5              # PT angle  [°]
pdmy_contractionParam1  = 0.1               # contrac1
pdmy_dilationParam1     = 0.2               # contrac3 (dilation param)
pdmy_dilationParam2     = 5.0               # dil1
pdmy_liquefactionParam1 = 10.0              # dil2
pdmy_liquefactionParam2 = 0.015             # liquefac1
pdmy_liquefactionParam4 = 1.0               # liquefac3
# Optional params (numYieldSurf, e, volLimits, cohesion) omitted — using OpenSees defaults
...
consol_n1     = 5                            # steps phase 1 (elastic)
consol_n2     = 3                            # steps phase 2 (plastic)
...
def _define_materials() -> None:
    """PressureDependMultiYield — 16-param minimum set (no optional params)."""
    ops.nDMaterial(
...
def run_dynamic(odb: "opst.post.CreateODB") -> None:
    ...
    num_cycles = 21  # 42 values: 0, -1, 1, -1, ...
    cycle_times = [float(t_start + i) for i in range(num_cycles * 2)]

    cycle_vals = [0.0]
    for i in range(20):
        cycle_vals.extend([-1.0, 1.0])
    cycle_vals.append(-1.0)  # 0, -1, 1, -1, ..., -1 (42 values)
...
def main() -> None:
    ...
    print("=" * 70)
    print("XMU Chapter 8.2 — Single quadUP Soil Element (Direct PDMY)")
