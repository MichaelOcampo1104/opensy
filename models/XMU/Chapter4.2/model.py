# ── 0. FILE HEADER ──────────────────────────────────────────────────────────────
"""
Model    : 2D Portal Frame — Steel01 Columns + Elastic Beam (Tabas EQ)
UniqueID : XMU_Chapter4_2
Author   : XMU (Xiamen University) — Chapter 4.2 Example
Date     : 2026-06-15
Purpose  : Textbook-example dynamic time-history analysis of a single-story
           single-bay portal frame with Steel01 bilinear columns and elastic
           beam under Tabas earthquake (Rayleigh damping 2% on mode 1).
Ref      : XMU Finite Element Analysis course, Chapter 4.2
Units    : N, mm, MPa  (see standards/units.py)
Notes    : Converted from model.tcl.
           Original units: kN, m, kPa — converted to N, mm, MPa.
           kN→N (×1000), m→mm (×1000), kPa→MPa (÷1000).
           Aggregator section materials act as force-deformation (not stress-strain):
           P stiffness ×1000 (kN→N), Mz stiffness ×1e9 (kN·m→N·mm, κ:1/m→1/mm).
           Regular elastic section: E÷1000, A×1e6, I×1e12 (standard SI→N-mm).
"""
...
# ── 2. TAG REGISTRY ──────────────────────────────────────────────────────────────
# Materials
MAT_STEEL_FLEX  = 2   # Steel01 — column flexural response
MAT_ELASTIC_AX  = 3   # Elastic  — column axial response

# Sections
SEC_COL  = 1   # Aggregator: P=MAT_ELASTIC_AX, Mz=MAT_STEEL_FLEX
...
# ── 3. PARAMETERS ────────────────────────────────────────────────────────────────
# --- Geometry (original: m) ---
frame_width  = 12.80 * m                            # bay width  [mm]
frame_height = 10.97 * m                            # column height  [mm]
n_ip         = 5                                    # integration points

# --- Column materials (original: kN/m² = kPa, acting as section force-deformation) ---
# Aggregator conversion: P=×1000 (kN→N), Mz=×1e9 (kN·m→N·mm, κ:1/m→1/mm)
E_col_ax = 4.62e7 * 1000.0                           # 4.62e10 N per unit strain
E_col_fx = 5.74e6 * 1.0e9                            # 5.74e15 N·mm per unit curvature
Fy_col   = 1.47e4 * 1.0e9                            # 1.47e13 N·mm yield moment
steel_b  = 0.01

# --- Beam elastic section (original: kN/m², m², m⁴) ---
E_beam = 2.49e7 / 1000.0                            # 24900 MPa
A_beam = 3.72 * m**2                                # cross-section area  [mm²]
I_beam = 1.8413 * m**4                              # moment of inertia  [mm⁴]

# --- Mass (original: kN·s²/m; ×1 → N·s²/mm) ---
mass_top = 80.0                                     # 80 N·s²/mm = 80 tonnes per node

# --- Beam uniform load (original: kN/m; ×1 → N/mm) ---
w_beam = -122.5                                     # downward uniform load  [N/mm]
...
def define_materials() -> None:
    """Steel01 for column flexure; Elastic for column axial."""
    ops.uniaxialMaterial("Steel01", MAT_STEEL_FLEX, Fy_col, E_col_fx, steel_b)
    ops.uniaxialMaterial("Elastic", MAT_ELASTIC_AX, E_col_ax)


# ── 6. SECTIONS ──────────────────────────────────────────────────────────────────
def define_sections() -> None:
    """Aggregator section for columns (P + Mz); Elastic section for beam."""
    ops.section("Aggregator", SEC_COL,
                MAT_ELASTIC_AX, "P", MAT_STEEL_FLEX, "Mz")
...
def define_elements() -> None:
    """Two nonlinearBeamColumn columns + one nonlinearBeamColumn beam."""
    ops.geomTransf("Linear", TRANS_COL)
    ops.geomTransf("Linear", TRANS_BEAM)
    ops.element("nonlinearBeamColumn", ELE_COL_L,
                NODE_L_BASE, NODE_L_TOP, n_ip, SEC_COL, TRANS_COL)
    ops.element("nonlinearBeamColumn", ELE_COL_R,
                NODE_R_BASE, NODE_R_TOP, n_ip, SEC_COL, TRANS_COL)
    ops.element("nonlinearBeamColumn", ELE_BEAM,
                NODE_L_TOP, NODE_R_TOP, n_ip, SEC_BEAM, TRANS_BEAM)
