...
set velocityFile velocityHistory.in
	...
	for {set j 1} {$j <= $nElemY($i)} {incr j 1} {
		for {set k 1} {$k <= $nElemX} {incr k} {
          ...
          # permeabilities are initially set at 1.0 m/s for gravity analysis, values are updated post-gravity
            element SSPquadUP [expr ($nElemX)*($j+$count-1) + $k] $nI $nJ $nK $nL $i $thick($i) $uBulk($i) 1.0 1.0 1.0 $eInit($i) 1.5e-6 $xWgt($i) $yWgt($i)
...
set dT 0.05
set nSteps  [expr int((100.0 -[getTime]) / $dT)]
set remStep $nSteps
puts "remStep $nSteps"
...
while {$success != -10} {
	...
	set success [analyze $remStep  $dT]
	if {$success == 0} {
		puts "PostShake Analysis Finished"
		break
	} else {
		set curTime  [getTime]
		puts "Analysis failed at $curTime . Try substepping."
