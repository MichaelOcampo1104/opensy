...
proc RunStaticLoading {LoadPatNum IDctrlNode IDctrlDOF LoadType LoadHistory Dincr numIter Tol ControlMethod} {
...
#	puts "Load Protocol:"
	...
	algorithm Newton
	analysis Static
	if {$ControlMethod == "Displacement"} {
		set currentLoad [nodeDisp $IDctrlNode $IDctrlDOF]
	} else {
		set currentLoad 0.0;
	}
	set currentIter 1;
	foreach Load $LoadProfile {
#		puts $Disp;
		set nextLoad $Load;
		set incrLoad [expr $nextLoad-$currentLoad];
		if {$ControlMethod == "Displacement"} {
			integrator DisplacementControl $IDctrlNode $IDctrlDOF $incrLoad
		} else {
			integrator LoadControl $incrLoad
		}		
