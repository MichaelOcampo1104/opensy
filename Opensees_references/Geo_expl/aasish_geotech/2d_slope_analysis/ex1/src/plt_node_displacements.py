...
def main():
    ...
    # fname_disp = "displacement.csv"
    fname_disp = "2ddex1_nodedisp_out.csv"
    fname_fig = "2ddex1_nodedisp.png"
    ...
    # node number
    nodetags = [5, 80, 6, 108]
    llbls = ["Slope toe", "Slope middle", "Slope top", "Far from slope"]
    colnames = ["Node_" + str(nodetag) for nodetag in nodetags]

    df = pd.read_csv(fdisp)
    df.plot(ax=ax, kind="line", x="dts", y=colnames, legend=False)
    ax.legend(labels=llbls)
    ax.grid(b=True, which="major", axis="both", ls=":", color="k", alpha=0.4)
    ax.set_xlim(right=15)
    ax.set_xlabel(r"$\Delta t$ [s]")
    ax.set_ylabel(r"X-dir disp. [m]")
