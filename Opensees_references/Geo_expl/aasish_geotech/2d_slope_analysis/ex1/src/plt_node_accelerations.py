...
def main():
    ...
    fname_disp = "2ddex1_nodeaccel_out.csv"
    fname_fig = "2ddex1_nodeaccel.png"
    ...
    llbls = ["Slope toe", "Mid slope", "Slope top", "Far from slope top"]
    df = pd.read_csv(fdisp)
    data = df.values
    for idx in range(1, len(nodetags) + 1):
        ax.plot(
            data[:, 0],
            data[:, idx] / 9.81,
            linestyle="-",
    ...
    ax.legend(labels=llbls, frameon=False)
    ax.set_xlabel(r"$t$ [s]")
    ax.set_ylabel(r"Horizontal acceleration, $a_\textrm{x}$ [g]")
