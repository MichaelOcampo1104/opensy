...
def get_data(fpath, ndof, nodetag):
    ...
    # 0 array indexing, therefore is nodetag is n then index is n - 1
...
def main():
    ...
    fnames = [
        "ex_g_02_siteresponse_totalstress_accel.out",
        "ex_g_02_siteresponse_totalstress_vel.out",
        "ex_g_02_siteresponse_totalstress_disp.out",
    ]
    ...
    fname_fig = "ex_g_02_siteresponse_totalstress_surface_groundmotion.png"
    ...
    for ax, fname, fname_ori in zip(axs, fnames, fnames_ori):
        ...
        ax.plot(dts, data, ls="-", alpha=0.5, label=label_py)
        ax.plot(dts_ori, data_ori, ls="-", alpha=0.5, label=label_tcl)
