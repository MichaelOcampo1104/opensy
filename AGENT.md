...
### §12ay — 3D `beamWithHinges` Six-Property Signature (E A Iz Iy G J); `ops.fix` Literal-vs-Unpack Arg-Count Quirk; Concentrated-vs-Distributed Plasticity A/B in One File (v1.49.0)

Source: `Dino_PlasticHinge` — elastic-plastic pushover of a 3D RC moment frame (2 bays × 2 bays × 4 stories, 30 nodes, 52 beams) converted from a Tcl PAIR (`co.tcl` = `beamWithHinges`, `co2.tcl` = `nonlinearBeamColumn`) that differ only in the `element` block. The repo's first `beamWithHinges` 3D model and first A/B two-variant conversion. Validation: **`nonlinearBeamColumn` 100/100 steps, node-2 UZ and base shear match `node2.out` to 0.00% / 0.00% (RMS 0.00000 mm)** — that Tcl generated the reference, so the exact match confirms a faithful conversion; **`beamWithHinges` 100/100 steps, 7.70% UZ / 1.05% base-shear diff** — the physically-expected gap between concentrated (slightly more flexible) and distributed plasticity.

#### 1. 3D `beamWithHinges` takes SIX elastic-interior properties (E A Iz Iy G J), not five

The Tcl form passes **six** values between the right hinge length (`lpJ`) and the transform tag:

```tcl
element beamWithHinges 1 1 2 1 500 1 500 2.000E+004 2.500E+005 5.208E+009 5.208E+009 8.333E+003 8.802E+009 1
#                                  eleTag i j secI lpI secJ lpJ E               A               Iz              Iy              G           J               transf
```

The OpenSeesPy wrapper accepts the **same six** — `ops.element("beamWithHinges", tag, i, j, secI, lpI, secJ, lpJ, E, A, Iz, Iy, G, J, transf)`. Empirically (this build), the **5-property form (`E A Iz Iy GJ`) and 4-property form (`E A Iz Iy`) both error**; only the full 6-property form builds. This differs from the 2D `beamWithHinges` precedent (Citiner, §12v) which takes only `E A Iz` for ndm=2.

```python
# 3D beamWithHinges — CORRECT (6 elastic-interior props, §12ay)
ops.element("beamWithHinges", tag, iNode, jNode,
            secI, lpI, secJ, lpJ,
            E, A, Iz, Iy, G, J,        # six elastic-interior stiffnesses
            transfTag)
```

**Rule:** For a 3D (ndm=3, ndf=6) `beamWithHinges`, pass all six elastic-interior properties `E A Iz Iy G J` verbatim from the Tcl — do not collapse to the 2D `E A Iz` form (Citiner) or the 5-property `… GJ` form. Detection: `OpenSeesError` at element construction with no preceding "invalid" message; cross-ref §12v (the `save_frame_resp=False` requirement, which still applies).

#### 2. `ops.fix(tag, 1,1,1,1,1,1)` errors on the LITERAL form, works on the UNPACK form

This openseespy build raises `OpenSeesError: See stderr output` (stderr: `WARNING: invalid # of constraint values`) on the **literal** positional form:

```python
ops.fix(1, 1, 1, 1, 1, 1)        # ❌ errors: "invalid # of constraint values"
```

but accepts the **unpack** form with an identical argument count:

```python
ops.fix(1, *[1, 1, 1, 1, 1, 1])  # ✓ works
vals = [1, 1, 1, 1, 1, 1]; ops.fix(1, *vals)   # ✓ works (the LifeDeath pattern)
```

This is a pure arg-counting quirk of the Python wrapper (it inspects the call shape differently for literal vs unpacked positional args), not a modelling issue — the constraint is applied identically either way. All existing repo models that parse fixity values into a list and call `ops.fix(tag, *vals)` (LifeDeath §12ax) already hit the working path; models written with literal args would fail.

**Rule:** Always call `ops.fix` via the unpack form `ops.fix(tag, *vals)` where `vals` is a 6-element list (for ndf=6) — never the literal `ops.fix(tag, 1, 1, 1, 1, 1, 1)`. Detection: `OpenSeesError` + stderr `WARNING: invalid # of constraint values` at the `ops.fix` call site. Cross-ref §12ax-3 (the same unpack form is what makes the orphan-repin work there).

#### 3. A/B two-variant conversion: one `model.py`, element-type switch, shared reference

When a source ships TWO Tcl variants of the same model that differ only in the element formulation (here: `co.tcl` beamWithHinges vs `co2.tcl` nonlinearBeamColumn — nodes/materials/sections/loads/analysis byte-identical), the faithful standardization is **one `model.py` with an element-type switch**, not two folders. Each variant is a full wipe/rebuild pushed over against the SAME reference curve, so the two plasticity formulations are directly comparable (this is the example's pedagogical point). Render the shared geometry (nodes/model/loads/pre-analysis viz) only on the first variant (the geometry is identical); render per-variant deformed-shape viz (slider/animation/peak) tagged by suffix.

```python
ELEMENT_TYPES = ["hinge", "fiber"]
for i, etype in enumerate(ELEMENT_TYPES):
    init_model(); define_materials(); define_sections()
    define_nodes_mass(src); define_boundary_conditions(src)
    define_transformations(src)
    if etype == "hinge":
        define_elements_hinge(src_hinge)      # beamWithHinges
    else:
        define_elements_fiber(src_fiber)      # nonlinearBeamColumn
    odb = create_odb(output_dir, odb_tag=ODB_TAG_HINGE if etype=="hinge" else ODB_TAG_FIBER)
    define_lateral_loads()
    ux, uz, shear = run_pushover(odb, ..., label)
```

**Rule:** Two-element-variant sources → one `model.py`, switch on element type, full rebuild per variant, one shared reference, three-way compare plot (variant-A vs variant-B vs ref). Validation expectation: the variant whose Tcl GENERATED the reference should match to ~0% (here `nonlinearBeamColumn`); the other carries the formulation's real physical gap (here 7.7% UZ). Cross-ref §12aw-5 (directional solver-mismatch gaps are NOT model errors — but here the gap is formulation-driven, not solver-driven, and is expected regardless of solver).

#### 4. Recorder `-time` under DisplacementControl = the load factor λ; base shear = λ × ΣP_lateral

The source `recorder Node -file node2.out -time -node 2 -dof 1 2 3 disp` records, per step: the pseudo-time (`-time`) and node-2's UX/UY/UZ. Under a `DisplacementControl` integrator, **pseudo-time = the load factor λ** by which the reference loads are scaled — so col 0 of `node2.out` is λ (0.0177 → 0.467 across the 100 steps), NOT a wall-clock time. The base shear is therefore `λ × ΣP_lateral` (here λ × 2.2 MN), reconstructed in post-processing. The controlled DOF (node-2 UX) is trivially 1 → 100 mm (the integrator target); the physically interesting, formulation-sensitive response is the **free companion DOF UZ** (0.011 → 3.94 mm), which is what the verification RMS is computed on.

**Rule:** A pushover reference recorded with `-time` under DisplacementControl gives λ in col 0; convert to base shear as `λ × ΣP_lateral`, and validate against the free companion DOFs (not the controlled one, which is trivially the target). Cross-ref §12ap-5 (recorder `-time` semantics), §12am (1:1 cadence via `static_split`).

#### Detection / Rules
- **3D `beamWithHinges` signature:** six elastic-interior props `E A Iz Iy G J` (not the 2D `E A Iz` of §12v/Citiner, nor a 5-prop `… GJ`). Detection: `OpenSeesError` at construction; `save_frame_resp=False` still required (§12v-1).
- **`ops.fix` literal-vs-unpack:** use `ops.fix(tag, *vals)`, never `ops.fix(tag, 1,1,1,1,1,1)`. Detection: `OpenSeesError` + `WARNING: invalid # of constraint values`.
- **A/B two-variant conversion:** one `model.py`, element-type switch, shared geometry viz, per-variant deformed viz, three-way compare plot. The Tcl-that-generated-the-reference variant should match ~0%.
- **`-time` under DisplacementControl = λ:** base shear = λ × ΣP_lateral; validate on free companion DOFs, not the controlled one.
